...
@section('content')
<div class="max-w-2xl">
    <h2 class="text-3xl font-bold text-gold mb-6">Nueva Categoría</h2>
    ...
    <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" required class="w-full bg-gray-900 border border-gold px-4 py-2 rounded text-white" placeholder="Ej: Programas de Recuperación">
        </div>

        <div>
            <label class="block text-gold font-bold mb-2">Descripción</label>
            <textarea name="description" required class="w-full bg-gray-900 border border-gold px-4 py-2 rounded text-white h-32" placeholder="Describe los objetivos y contenido de esta categoría"></textarea>
        </div>
