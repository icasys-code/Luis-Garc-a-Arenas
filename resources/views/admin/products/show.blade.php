    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <p class="text-gray-900">{{ $product->description ?? 'N/A' }}</p>
            </div>

            <div>
                <label class="block text-gray-600 text-sm font-bold mb-2">Categoría</label>
                <p class="text-gray-900">{{ $product->categorie->name ?? 'N/A' }}</p>
            </div>

            <div>
                <label class="block text-gray-600 text-sm font-bold mb-2">Precio</label>
                <p class="text-gray-900 text-lg font-bold">${{ number_format($product->price, 2) }}</p>
            </div>
        ...
        <div class="flex items-center justify-between mt-8 pt-6 border-t">
            ...
            <form action="{{ route('admin.products.destroy', $product) }}" method="POST" style="display:inline;" onclick="return confirm('¿Estás seguro de que deseas eliminar este producto?')">
                @csrf
                @method('DELETE')
