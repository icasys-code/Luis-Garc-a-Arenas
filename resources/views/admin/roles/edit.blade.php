...
@section('content')
<div class="max-w-2xl">
    <h2 class="text-3xl font-bold text-gold mb-6">Editar Rol</h2>
    ...
    <form action="{{ route('admin.roles.update', $role->id) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" required value="{{ $role->name }}" class="w-full bg-gray-900 border border-gold px-4 py-2 rounded text-white">
        </div>
